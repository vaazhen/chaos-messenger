const FINGERPRINT_WORDS = [
  ...
  'beef', 'before', 'begin', 'behave', 'behind', 'believe', 'below', 'belt', 'bench', 'best',
  ...
  'caught', 'cause', 'caution', 'cave', 'ceiling', 'celery', 'cement', 'census', 'century', 'ceo',
  ...
  'impact', 'impose', 'improve', 'impulse', 'include', 'income', 'increase', 'index', 'indicate', 'indoor',
  ...
  'obvious', 'occur', 'ocean', 'october', 'odor', 'off', 'offense', 'offer', 'office', 'often',
  ...
  'stage', 'stairs', 'stamp', 'stand', 'start', 'state', 'stay', 'steak', 'steel', 'steep',
  'steer', 'stem', 'step', 'stereo', 'stick', 'still', 'sting', 'stock', 'stomach', 'stone',
  ...
  'suggest', 'suit', 'sun', 'sunny', 'sunset', 'super', 'supply', 'support', 'suppose', 'sure',
  'surface', 'surgery', 'surprise', 'surround', 'survey', 'suspect', 'sustain', 'swallow', 'swamp', 'swap',
  ...
  'wealth', 'weapon', 'wear', 'weasel', 'weather', 'web', 'wedding', 'week', 'weird', 'welcome',
...
export async function computeContactSafetyNumber(ownIdentityKey, theirIdentityKeys, extraKeys = []) {
  ...
  for (let i = 0; i < 12; i++) {
    const idx = (hashBytes[i * 2] << 8 | hashBytes[i * 2 + 1]) % FINGERPRINT_WORDS.length;
    wordParts.push(FINGERPRINT_WORDS[idx]);
  }
  ...
  for (let i = 0; i < 3; i++) {
    const val = (hashBytes[i * 2] * 256 + hashBytes[i * 2 + 1]) % 10000;
    numericParts.push(val.toString().padStart(4, '0'));
  }

  return {
    fingerprint: formatWordFingerprint(wordParts),
    wordList: wordParts,
    hexList: hexParts,
    numericBlocks: numericParts,
    bytes: hashBytes,
  };
}

function formatWordFingerprint(words) {
  const groups = [];
...
export function formatSafetyNumber(safety) {
  return {
    numeric: safety.numericBlocks.join(' '),
    hex: safety.hexList.join(':'),
    words: safety.fingerprint,
  };
}

export function areSafetyNumbersEqual(a, b) {
  if (!a || !b) return false;
  if (a.bytes && b.bytes) {
    if (a.bytes.length !== b.bytes.length) return false;
    return a.bytes.every((v, i) => v === b.bytes[i]);
  }
